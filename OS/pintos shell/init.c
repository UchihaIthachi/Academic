  ...
  while (1) {
    ...
    while ((c = input_getc()) != '\r') {
        if (c == '\b') { // Backspace key
            if (index > 0) {
                // Move back one character and overwrite with a space
                putchar('\b');
                putchar(' ');
                putchar('\b');
                input[--index] = '\0';
            ...
            // Display the character and store it in the input buffer
            putchar(c);
            input[index++] = c;
        }
    }
    ...
    if (strcmp(input, "hello") == 0) {
        ...
        int64_t t = ticks / TIMER_FREQ;
        printf("Time since boot: %.lld seconds.\n", t);
        ...
        size_t total_available_ram = free_end - free_start;

        printf("Available RAM: %zu bytes\n", total_available_ram);

    } else if (strcmp(input, "priority") == 0) {
        printf("The thread priority of the current thread: %d\n", thread_get_priority());
    } else if (strcmp(input, "exit") == 0) {
        puts("Exiting shell...!");
        thread_exit();
    } else if (strcmp(input, "whoami") == 0) {
        puts("Harshana (210167E)");
    } else {
